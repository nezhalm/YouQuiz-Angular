...
public class StudentController {
    ...
    @GetMapping()
    public ResponseEntity<Map<String, Object>> students(@RequestParam int page, @RequestParam int items) throws Exception {
        Map<String, Object> message = new HashMap<>();
        try{
            if(studentService.findAll(page,items).isEmpty()) {
                message.put("message", "No students found!");
                return new ResponseEntity<>(message, HttpStatus.OK);
            }
            message.put("message", "students found");
            message.put("students", studentService.findAll(page,items));
            return new ResponseEntity<>(message, HttpStatus.OK);
        }catch(Exception e){
